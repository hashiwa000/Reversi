...
public abstract class EdgePrefferingPlayer extends AbstractPlayer {

  private final List<Set<RCell>> preferred;

  public EdgePrefferingPlayer(RManager manager) {
    super(manager);
    preferred = getPrefferedSet();
  }

  protected abstract List<Set<RCell>> getPrefferedSet();

  @Override
  public RCell next() {
    // preffered
    for (Set<RCell> set: preferred) {
      for (RCell c: set) {
        if (getManager().containsInSelectable(c)) {
          return c;
        }
      }
    }
